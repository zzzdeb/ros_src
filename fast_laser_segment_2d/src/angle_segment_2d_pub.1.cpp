...
void AngleSegment2dPub::scan_callback(const sensor_msgs::LaserScan::ConstPtr& scan_msg)
        {
            ...
            ROS_INFO("now");
            shared_ptr<vector<vector<int> > > labels = angle_segment_2d(labels);
            ///Debug
            // labels->clear();
            ...
            ROS_DEBUG("%i segmentation", labels.size());
        }
...
void AngleSegment2dPub::publish_scans(){
    if (scan_pubs.size()<segmented_scans.size())
    {
        ...
        for(unsigned int i=0; i<scan_pubs.size(); i++){
            ...
            out << i;
            scan_pubs[i] = nh_.advertise<LaserScan>(out.str(),1);
        }
    } // resizing
...
vector<vector<int> > AngleSegment2dPub::angle_segment_2d(const shared_ptr<vector<vector<int> > >& output)
{

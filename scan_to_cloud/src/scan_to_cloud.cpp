...
class ScanToCloud{
    ...
    public:
        ScanToCloud(){
            ROS_INFO("ScanToCloud constructed");
            ...
            cloud_pub = nh.advertise<sensor_msgs::PointCloud2>("scan_to_cloud",1);
            scan_sub = nh.subscribe("scan", 1, &ScanToCloud::callback, this);
        }

        ~ScanToCloud(){
            ROS_INFO("ScanToCloud destructed");
        }
